 ...
 * mimics a very basic user interface to control a home lilbug such as a washing
 * machine or dryer.
 *
 ...
 * to enable a user to connect the lilbug to the home network, it then transitions
 ...
 *
 * Once connected, the lilbug configuration web page automatically appears in the
 ...
 *
 * After connection to the home network, the app starts an lilbug web server that
 * displays a web page with clickable buttons. A Wi-Fi client connected to the same home
 * Wi-Fi network as the WICED module connects to the lilbug web server using a web browser.
 * The lilbug app does not currently support L3 discovery (such as Bonjour or uPnP)
...
static void run_sta_web_server( void )
{
    ...
    if ( lilbug_config.config_type == CONFIG_WPS_PBC )
    {
        do_wps( WICED_WPS_PBC_MODE, "" );
    }
    else if ( lilbug_config.config_type == CONFIG_WPS_PIN )
    {
        do_wps( WICED_WPS_PIN_MODE, lilbug_config.vals.wps_pin.pin );
    }
    else
#endif /* ifdef APPLIANCE_ENABLE_WPS */
    {
        if ( lilbug_config.config_type == CONFIG_SCANJOIN )
        {
            /* Join a wireless network */
            while ( WWD_SUCCESS != wwd_wifi_join_specific( &lilbug_config.vals.scanjoin.scanresult, (uint8_t*) lilbug_config.vals.scanjoin.passphrase, lilbug_config.vals.scanjoin.passphrase_len, NULL, WWD_STA_INTERFACE ) )
            {
