...
#define BISCUIT_TRIG_OUT_A_PORT GPIOB
#define BISCUIT_TRIG_OUT_A_PIN  GPIO_Pin_7 // D0
#define BISCUIT_TRIG_OUT_B_PORT GPIOB
#define BISCUIT_TRIG_OUT_B_PIN  GPIO_Pin_6 // D1
#define BISCUIT_TRIG_IN_A_PORT GPIOB
#define BISCUIT_TRIG_IN_A_PIN  GPIO_Pin_5 // D2
#define BISCUIT_TRIG_IN_B_PORT GPIOB
#define BISCUIT_TRIG_IN_B_PIN  GPIO_Pin_4 // D3
#define BISCUIT_DAC_CS_PORT GPIOC
#define BISCUIT_DAC_CS_PIN  GPIO_Pin_6 // A2
#define BISCUIT_DAC_SCK_PORT GPIOA
#define BISCUIT_DAC_SCK_PIN  GPIO_Pin_5 // A3
#define BISCUIT_DAC_SDI_PORT GPIOA
#define BISCUIT_DAC_SDI_PIN  GPIO_Pin_7 // A5
...
static void spi_transfer(uint8_t working) {
  ...
  for(int i = 1; i <= 8; i++) {
    if(working > 127)
      setPin(BISCUIT_DAC_SDI_PORT, BISCUIT_DAC_SDI_PIN); // if it is a 1 (ie. B1XXXXXXX), set the master out pin high
    else
      clearPin(BISCUIT_DAC_SDI_PORT, BISCUIT_DAC_SDI_PIN); // if it is not 1 (ie. B0XXXXXXX), set the master out pin low
    setPin(BISCUIT_DAC_SCK_PORT, BISCUIT_DAC_SCK_PIN); // set clock high, the pot IC will read the bit into its register
    ...
    clearPin(BISCUIT_DAC_SCK_PORT, BISCUIT_DAC_SCK_PIN);
  }
}

static void spi_out(uint8_t high, uint8_t low) {
  /* digitalWrite(DAC_CS_PIN, LOW); */
  clearPin(BISCUIT_DAC_CS_PORT, BISCUIT_DAC_CS_PIN);
  spi_transfer(high);
  spi_transfer(low);
  setPin(BISCUIT_DAC_CS_PORT, BISCUIT_DAC_CS_PIN);
  /* digitalWrite(DAC_CS_PIN, HIGH); */
}

static void spi_init() {
  configureDigitalOutput(BISCUIT_DAC_CS_PORT, BISCUIT_DAC_CS_PIN);
  configureDigitalOutput(BISCUIT_DAC_SCK_PORT, BISCUIT_DAC_SCK_PIN);
  configureDigitalOutput(BISCUIT_DAC_SDI_PORT, BISCUIT_DAC_SDI_PIN);
